...
def parse_and_update_json(input_txt, key_names, input_key_name_group, input_key_assignment):
    ...
    def find_key_in_groups(key, key_name_group, assign_key_to_subst):
        """Finds the actual key name from key name groups."""
        for group in key_name_group:
            if key in group:
                for k in group:
                    if k in assign_key_to_subst:
                        return k
        return None
    ...
    # Construct key_assignment subst mappings
    assign_key_to_subst = {}
    subst_to_assign_key = {}
    for key, value in key_assignments.items():
        if value["subst"]:
            assign_key_to_subst[key] = value["subst"]
            subst_to_assign_key[value["subst"]] = key
    ...
    for line in lines:
        ...
        try:
            ...
            # Find the actual key name from key name groups if not found directly, using subst mapping
            if key_raw not in assign_key_to_subst:
                key = find_key_in_groups(key_raw, key_name_group, assign_key_to_subst)
                if not key:
            ...
            if value_key_name_raw not in key_names:
                value_key_name = find_key_in_groups(value_key_name_raw, key_name_group, assign_key_to_subst)
            ...
            # Update key assignments, using subst mapping to find the actual key
            actual_key = subst_to_assign_key.get(key, key)  # If no subst, use the key itself
...
key_names = [
    "A", "Atmark", "B", "BS", "C", "Caret", "CloseBracket", "Colon", "Comma",
